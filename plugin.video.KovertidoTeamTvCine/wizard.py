...
import xbmc, xbmcaddon, xbmcgui, xbmcplugin, os, sys, xbmcvfs, glob, json
import shutil
import errno
import string
import random
...
def toggleAdult():
	...
	if len(found) > 0:
		...
		forceUpdate(True)
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Addon de Adultos no encontrado[/COLOR]" % COLOR2)
...
def cleanupBackup():
	...
	list = []; filelist = []
	if len(folder) == 0:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Carpeta de Backups: vacia[/COLOR]" % (COLOR2))
		return
	...
	selected = DIALOG.select("%s: Elige los archivos a borrar de 'MisBuilds'." % ADDONTITLE, list)
	...
	if selected == -1:
		...
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres borrar [COLOR %s]%s[/COLOR] de la carpeta 'MisBuilds'?[/COLOR]" % (COLOR2, COLOR1, list[selected]), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), yeslabel="[B][COLOR springgreen]Borrar[/COLOR][/B]", nolabel="[B][COLOR red]No, Cancelar[/COLOR][/B]"):
			if os.path.isfile(path):
				...
				try:
					...
					log("Error borrando %s" % path, xbmc.LOGNOTICE)
			if passed: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]%s Borrado![/COLOR]" % (COLOR2, list[selected]))
			else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Error Borrando %s![/COLOR]" % (COLOR2, list[selected]))
		else:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Borrar cancelado![/COLOR]" % COLOR2)
	...
	for root, dirs, files in os.walk(url):
		for file in files:
			...
			if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
				DP.update(perc, "[COLOR %s]Escaneando: [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, root.replace(HOME, '')), "[COLOR %s]%s[/COLOR]" % (COLOR1, file), "Espera[/COLOR]")
...
def clearCrash():
	...
	for file in glob.glob(os.path.join(LOG, '*crashlog*.*')):
		files.append(file)
	if len(files) > 0:
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar el Crash logs?' % COLOR2, '[COLOR %s]%s[/COLOR] Archivos Encontrados[/COLOR]' % (COLOR1, len(files)), yeslabel="[B][COLOR springgreen]Borrar Logs[/COLOR][/B]", nolabel="[B][COLOR red]Mantener Logs[/COLOR][/B]"):
			for f in files:
				os.remove(f)
			LogNotify('[COLOR %s]Borrar Crash Logs[/COLOR]' % COLOR1, '[COLOR %s]%s Crash Logs Borrados[/COLOR]' % (COLOR2, len(files)))
		else: LogNotify('[COLOR %s]%s[/COLOR]' % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Crash Logs Cancelado[/COLOR]' % COLOR2)
	else: LogNotify('[COLOR %s]Borrar Crash Logs[/COLOR]' % COLOR1, '[COLOR %s]No hay Crash Logs[/COLOR]' % COLOR2)

def hidePassword():
	if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]ocultar[/COLOR] todas las passwords cuando escribes en los ajustes de addons[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Ocultar Passwords[/COLOR][/B]", nolabel="[B][COLOR red]No, Cancelar[/COLOR][/B]"):
		count = 0
	...
	if workingURL(WIZARDFILE):
		...
		if ver > VERSION:
			...
			if yes:
				...
				DP.create(ADDONTITLE,'[COLOR %s]Descargando Actualizacion...' % COLOR2,'', 'Espera[/COLOR]')
...
def asciiCheck(use=None, over=False):
	...
	if source == "":
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]ASCII Cancelado[/COLOR]" % COLOR2)
		return
	...
	for base, dirs, files in os.walk(source):
		...
		for file in files:
			...
			DP.update(prog2,"[COLOR %s]Comprobando archivos no ASCII" % COLOR2,'[COLOR %s]%s[/COLOR]' % (COLOR1, d), 'Espera[/COLOR]')
			try:
				file.encode('ascii')
	...
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]ASCII No encontrado[/COLOR]" % COLOR2)

def fileCount(home, excludes=True):
	exclude_dirs  = [ADDON_ID, 'cache', 'system', 'packages', 'Thumbnails', 'peripheral_data', 'temp', 'MisBuilds', 'library', 'keymaps']
...
def backUpOptions(type, name=""):
	exclude_dirs  = [ADDON_ID, 'cache', 'system', 'Thumbnails', 'peripheral_data', 'temp', 'MisBuilds', 'keymaps', 'cdm']
	exclude_files = ['Textures13.db', '.DS_Store', 'advancedsettings.xml', 'Thumbs.db', '.gitignore']
	## TODO: fix these
	bad_files     = [
					(os.path.join(ADDOND, 'plugin.video.placenta', 'cache.db')),
	...
	try:
		...
		return
	if type == "addon pack":
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres hacer un pack de addons?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Cancelar Pack[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Crear Pack[/COLOR][/B]"):
			if name == "":
				name = getKeyboard("","Introduce un nombre para tu pack %s zip" % type)
			...
			for folder in sorted(fold, key = lambda x: x):
				...
				if os.path.exists(xml):
					...
					if len(match) > 0:
						...
						addonfolds.append(foldername)
			if KODIV > 16:
				selected = DIALOG.multiselect("%s: Escoge los addons que quieres guardar." % ADDONTITLE, addonnames)
				if selected == None: selected = []
			else:
				selected = []; choice = 0
				tempaddonnames = ["-- Click aqui para Continuar --"] + addonnames
				while not choice == -1:
					choice = DIALOG.select("%s: Elige los addons que quieres guardar." % ADDONTITLE, tempaddonnames)
					if choice == -1: break
					elif choice == 0: break
					else:
						choice2 = (choice-1)
			...
			log(selected)
			DP.create(ADDONTITLE,'[COLOR %s][B]Creando archivo zip:[/B][/COLOR]' % COLOR2,'', 'Espera')
			if len(selected) > 0:
				added = []
			...
			DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR] [COLOR %s]Pack completado:[/COLOR]" % (COLOR1, name, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, zipname))
	elif type == "build":
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres guardar una copia de la build actual?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Cancelar Copia[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Crear Copia[/COLOR][/B]"):
			if name == "":
				name = getKeyboard("","Por favor introduce un nombre para la copia %s zip" % type)
				if not name: return False
			...
			for_progress  = 0
			ITEM          = []
			if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres incluir la carpeta addon_data?" % COLOR2, 'Esta contiene [COLOR %s]TODOS[/COLOR] los ajustes de los addons. [COLOR %s]ES RECOMENDABLE[/COLOR] Guardarlo para el buen funcionamiento de los addons.' % (COLOR1, COLOR1), '[COLOR %s]%s[/COLOR] addon_data se obvia[/COLOR]' % (COLOR1, ADDON_ID), yeslabel='[B][COLOR springgreen]Incluir data[/COLOR][/B]',nolabel='[B][COLOR red]No incluir Data[/COLOR][/B]'):
			...
			try:
				...
				try:
					...
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]We are unable to write to the current backup directory, would you like to change the location?[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Change Directory[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]"):
						...
						return
			DP.create("[COLOR %s]%s[/COLOR][COLOR %s]: Creando zip[/COLOR]" % (COLOR1, ADDONTITLE,COLOR2), "[COLOR %s]Creando copia del zip" % COLOR2, "", "Espera...[/COLOR]")
			for base, dirs, files in os.walk(HOME):
				dirs[:] = [d for d in dirs if d not in exclude_dirs]
				files[:] = [f for f in files if f not in exclude_files]
				for file in files:
			...
			for base, dirs, files in os.walk(HOME):
				...
				for file in files:
					try:
						...
						DP.update(int(progress), '[COLOR %s]Creando copia del zip: [COLOR%s]%s[/COLOR] / [COLOR%s]%s[/COLOR]' % (COLOR2, COLOR1, for_progress, COLOR1, N_ITEM), '[COLOR %s]%s[/COLOR]' % (COLOR1, file), '')
						fn = os.path.join(base, file)
						...
						try:
							...
							#log("%s " % (Exception as e))
						if DP.iscanceled():
							DP.close()
							LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Backup Cancelado[/COLOR]" % COLOR2)
							sys.exit()
					except Exception as e:
						log("[Back Up] Type = '%s': Unable to backup %s" % (type, file), xbmc.LOGNOTICE)
		...
		if not DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Copia del Skin[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Quieres crear una Copia del Skin?[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Continuar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"): LogNotify("Theme Backup", "Cancelled!"); return False
		if name == "":
		...
		try:
				...
				if 'script.skinshortcuts' in match:
						for base, dirs, files in os.walk(os.path.join(ADDOND,'script.skinshortcuts')):
							...
							for file in files:
								fn = os.path.join(base, file)
								zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Skin y Menu Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Quieres incluir las imagenes del skin de la carpeta [COLOR %s]Backgrounds[/COLOR]?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR springgreen]Si[/COLOR][/B]", nolabel="[B][COLOR red]No, Continuar[/COLOR][/B]"):
			...
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Skin y Menu Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Quieres incluir algun addon?[/COLOR]" % (COLOR2), yeslabel="[B][COLOR springgreen]Si[/COLOR][/B]", nolabel="[B][COLOR red]No, continuar[/COLOR][/B]"):
				fold = glob.glob(os.path.join(ADDONS, '*/'))
				addonnames = []; addonfolds = []
				for folder in sorted(fold, key = lambda x: x):
					foldername = os.path.split(folder[:-1])[1]
					...
					if os.path.exists(xml):
						...
						if len(match) > 0:
							...
							addonfolds.append(foldername)
				if KODIV > 16:
					selected = DIALOG.multiselect("%s: Elige los addons." % ADDONTITLE, addonnames)
					if selected == None: selected = []
				else:
					selected = []; choice = 0
					tempaddonnames = ["-- Click here to Continue --"] + addonnames
					while not choice == -1:
						choice = DIALOG.select("%s: Elige los addons." % ADDONTITLE, tempaddonnames)
						if choice == -1: break
						elif choice == 0: break
						else:
							choice2 = (choice-1)
							...
							for depends in match:
								...
								for base, dirs, files in os.walk(os.path.join(ADDONS,depends)):
									...
									for file in files:
										...
										zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
										added.append(depends)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Copia de Skin[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Quieres incluir los ajustes de Kodi [COLOR %s]guisettings.xml[/COLOR]?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR springgreen]Incluir[/COLOR][/B]", nolabel="[B][COLOR red]No, Continuar[/COLOR][/B]"):
		...
		if not tempzipname == '':
			...
			if success == 0:
				xbmcvfs.copy(tempzipname, zipname)
				xbmcvfs.delete(tempzipname)
		DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] zip del skin hechos:[/COLOR]" % (COLOR1, themename, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, zipname))
	elif type == "addondata":
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres hacer una copia de la carpeta addon_data?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Cancelar[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Crear Copia[/COLOR][/B]"):
			if name == "":
			...
			DP.create("[COLOR %s]%s[/COLOR][COLOR %s]: Creando Zip[/COLOR]" % (COLOR1, ADDONTITLE,COLOR2), "[COLOR %s]Creando copia en zip" % COLOR2, "", "Espera...[/COLOR]")
			...
			for base, dirs, files in os.walk(ADDOND):
				...
				for file in files:
					try:
						for_progress += 1
						progress = percentage(for_progress, N_ITEM)
						DP.update(int(progress), '[COLOR %s]Creando copia en zip: [COLOR%s]%s[/COLOR] / [COLOR%s]%s[/COLOR]' % (COLOR2, COLOR1, for_progress, COLOR1, N_ITEM), '[COLOR %s]%s[/COLOR]' % (COLOR1, file), '')
						fn = os.path.join(base, file)
						if file in LOGFILES: log("[Back Up] Type = '%s': Ignore %s" % (type, file), xbmc.LOGNOTICE); continue
						elif os.path.join(base, file) in bad_files: log("[Back Up] Type = '%s': Ignore %s" % (type, file), xbmc.LOGNOTICE); continue
						elif os.path.join('addons', 'packages') in fn: log("[Back Up] Type = '%s': Ignore %s" % (type, file), xbmc.LOGNOTICE); continue
...
def restoreLocal(type):
	...
	try:
		...
		DIALOG.ok(ADDONTITLE, "[COLOR %s]Error making Back Up directories:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, str(e)))
		return
	file = DIALOG.browse(1, '[COLOR %s]Elige la copia que quieres restaurar[/COLOR]' % COLOR2, 'files', '.zip', False, False, mybuilds)
		...
		try:
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]Borrar Paquetes descargados" % COLOR2, "[COLOR %s]%s[/COLOR] archivos encontrados / [COLOR %s]%s[/COLOR] de peso." % (COLOR1, str(file_count), COLOR1, size), "Quieres Borrarlos?[/COLOR]", nolabel='[B][COLOR red]No borrar[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Borrar Paquetes[/COLOR][/B]')
					if yes:
						for f in files: os.unlink(os.path.join(root, f))
						for d in dirs: shutil.rmtree(os.path.join(root, d))
						LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Paquetes Borrados[/COLOR]' % COLOR2)
				else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]No hay paquetes[/COLOR]' % COLOR2)
		except Exception as e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Error borrando paquetes![/COLOR]' % COLOR2)
			log("Error al borrar paquetes: %s" % str(e), xbmc.LOGERROR)
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]No hay paquetes[/COLOR]' % COLOR2)
...
def clearPackagesStartup():
	...
	if os.path.exists(PACKAGES):
		...
		try:
			...
			if file_count > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Paquetes Terminado: %s[/COLOR]' % (COLOR2, convertSize(cleanupsize)))
			else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]No hay Paquetes[/COLOR]' % COLOR2)
		except Exception as e:
						...
						for table in textexe.fetchall():
							try:
								...
								try:
									...
									pass
						textexe.close()
		else: log("Borrar Cache: Borrar Video Cache No Activado", xbmc.LOGNOTICE)
	LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Cache: Borrados %s Archivos[/COLOR]' % (COLOR2, delfiles))
...
def checkRepos():
	...
	if len(badrepos) > 0:
		...
		msg += '[/COLOR]'
		TextBox("%s: Repositorios Malos" % ADDONTITLE, msg)
	else:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Todos Funcionando![/COLOR]" % COLOR2)
...
def killxbmc(over=None):
	if over: choice = 1
	else: choice = DIALOG.yesno('Forzar Cierre de Kodi', '[COLOR %s]Vas a Cerrar Kodi' % COLOR2, 'Quieres continuar?[/COLOR]', nolabel='[B][COLOR red]Cancelar[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Continuar[/COLOR][/B]')
	if choice == 1:
...
def purgeDb(name):
	...
	LogNotify("[COLOR %s]Purgar Base de Datos[/COLOR]" % COLOR1, "[COLOR %s]%s Completada[/COLOR]" % (COLOR2, show[len(show)-1]))
...
def oldThumbs():
	...
	if len(images) > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Thumbs: %s Archivos / %s MB[/COLOR]!' % (COLOR2, str(len(images)), removed))
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Thumbs: No hay[/COLOR]' % COLOR2)
