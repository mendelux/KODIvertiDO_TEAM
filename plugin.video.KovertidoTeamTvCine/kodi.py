...
Addonsatle = addon.getAddonInfo('name')
...
def LogNotify(satle, message, times, icon):
    xbmc.executebuiltin("XBMC.Notification(" + satle + "," + message + "," + times + "," + icon + ")")
...
def addDir(name, url, mode, thumb, cover=None, fanart=fanart, meta_data=None, is_folder=None,
    ...
    list_item = xbmcgui.Lissatem(name, iconImage=thumb, thumbnailImage=thumb)
    list_item.setProperty('fanart_image', fanart)
    if meta_data is None:
        list_item.setInfo('video', {'satle': list_item.getLabel(), 'plot': description})
...
def addItem(name, url, mode, iconimage, fanart=fanart, description=None):
    ...
    liz = xbmcgui.Lissatem(name, u, iconImage="DefaultFolder.png", thumbnailImage=iconimage)
    liz.setInfo('video', {'satle': liz.getLabel(), 'plot': description})
    liz.setProperty("fanart_image", fanart)
    liz.setArt({'poster': iconimage, 'fanart_image': fanart, 'banner': 'banner.png'})
    ok = xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=u, lissatem=liz, isFolder=False)
    # xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=u, lissatem=liz, isFolder=False)
    return ok


def create_item(queries, label, thumb='', fanart='', is_folder=None, is_playable=None, total_items=0, menu_items=None,
                replace_menu=False):
    list_item = xbmcgui.Lissatem(label, iconImage=thumb, thumbnailImage=thumb)
...
def add_item(queries, list_item, fanart='', is_folder=None, is_playable=None, total_items=0, menu_items=None,
    ...
    list_item.setInfo('video', {'satle': list_item.getLabel()})
    list_item.setProperty('isPlayable', playable)
